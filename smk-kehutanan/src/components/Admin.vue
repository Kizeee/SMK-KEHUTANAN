<template>
  <div class="min-h-screen bg-gray-100 flex items-center justify-center p-4">
    <div class="w-full max-w-3xl bg-white p-8 rounded-xl shadow-lg relative">
      <button
        @click="handleLogout"
        class="absolute top-4 right-4 text-sm text-gray-500 hover:text-red-600 hover:underline transition-colors"
      >
        Logout
      </button>
      ...
      <div class="border-b border-gray-200 mb-6">
        <nav class="-mb-px flex space-x-6" aria-label="Tabs">
          <button
            ...
            :class="['tab-button', { 'tab-active': activeTab === 'slider' }]"
          >
            Kelola Slider
          </button>
          <button
            @click="activeTab = 'berita'"
            :class="['tab-button', { 'tab-active': activeTab === 'berita' }]"
          >
            Kelola Berita
          </button>
          <button
            @click="activeTab = 'galeri'"
            :class="['tab-button', { 'tab-active': activeTab === 'galeri' }]"
          >
            Kelola Galeri
          </button>
          <button
            @click="activeTab = 'guru'"
            :class="['tab-button', { 'tab-active': activeTab === 'guru' }]"
          >
            Kelola Guru
          </button>
        </nav>
      ...
      <div v-if="activeTab === 'slider'">
        <form
          @submit.prevent="handleSubmit('slider')"
          class="mb-12 p-4 border rounded-lg bg-gray-50"
        >
          <h2 class="text-xl font-semibold text-gray-800 mb-4">
            {{ editMode ? "Edit Slide" : "Tambah Slide Baru" }}
          </h2>
          <div class="mb-4">
            <label class="form-label"
              >Judul Slide (gunakan tag span untuk warna)</label
            >
            <input
              type="text"
          ...
          <div class="grid grid-cols-2 gap-4 mb-4">
            ...
            <div>
              <label :for="`sliderFile`" class="form-label"
                >Gambar Latar (Kosongkan jika tidak ubah)</label
              >
              <input
                type="file"
                @change="handleFileChange($event, 'slider')"
                :id="`sliderFile`"
        ...
        <div v-else class="space-y-4 max-h-96 overflow-y-auto pr-2">
          ...
          >
            <div class="flex items-center min-w-0">
              <img
                :src="item.imageUrl"
                class="w-20 h-12 object-cover rounded mr-4 flex-shrink-0"
              />
              <div class="min-w-0">
                <p
            ...
            <div class="flex space-x-3 flex-shrink-0 ml-4">
              <button
                @click="startEdit(item, 'slider')"
                class="text-sm text-blue-600 hover:underline"
              >
                Edit
              </button>
              <button
                @click="handleDelete('hero_slider', item.id)"
      ...
      <div v-if="activeTab === 'berita'">
        <form
          @submit.prevent="handleSubmit('berita')"
          class="mb-12 p-4 border rounded-lg bg-gray-50"
        >
          <h2 class="text-xl font-semibold text-gray-800 mb-4">
          ...
          <div class="mb-4">
            <label :for="`beritaFile`" class="form-label"
              >Gambar Utama (Kosongkan jika tidak ingin mengubah)</label
            >
            <input
              type="file"
              @change="handleFileChange($event, 'berita')"
              :id="`beritaFile`"
              class="form-file-input"
            />
          </div>
        ...
        <div v-else class="space-y-4 max-h-96 overflow-y-auto pr-2">
          ...
          >
            <div class="flex items-center min-w-0">
              <img
                :src="item.imageUrl"
                class="w-16 h-12 object-cover rounded mr-4 flex-shrink-0"
              />
            ...
            <div class="flex space-x-3 flex-shrink-0 ml-4">
              <button
                @click="startEdit(item, 'berita')"
                class="text-sm text-blue-600 hover:underline"
              >
                Edit
              </button>
              <button
                @click="handleDelete('berita', item.id)"
                class="text-sm text-red-600 hover:underline"
              >
                Hapus
              </button>
      ...
      <div v-if="activeTab === 'galeri'">
        <form
          @submit.prevent="handleSubmit('galeri')"
          class="mb-12 p-4 border rounded-lg bg-gray-50"
        >
          <h2 class="text-xl font-semibold text-gray-800 mb-4">
            {{ editMode ? "Edit Item Galeri" : "Unggah Gambar Baru" }}
          </h2>
          <div class="mb-4">
            <label for="galeriTitle" class="form-label">Judul Gambar</label>
            <input
              type="text"
              v-model="galeri.title"
              id="galeriTitle"
              class="form-input"
              required
            />
          ...
          <div class="mb-4">
            <label :for="`galeriFile`" class="form-label"
              >Pilih Gambar (Kosongkan jika tidak ingin mengubah)</label
            >
            <input
              type="file"
              @change="handleFileChange($event, 'galeri')"
              :id="`galeriFile`"
              class="form-file-input"
            />
          </div>
        ...
        <div
          v-else
          class="grid grid-cols-2 md:grid-cols-3 gap-4 max-h-96 overflow-y-auto pr-2"
        >
          <div
            v-for="item in daftarGaleri"
            :key="item.id"
          ...
          >
            ...
            />
            <div
              class="absolute inset-0 bg-black/60 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity rounded-lg"
            >
              <p class="text-white text-center text-sm mb-2 px-1">
                {{ item.title }}
              </p>
              <div class="flex space-x-3">
                <button
                  @click="startEdit(item, 'galeri')"
                  class="text-xs text-white bg-blue-600 hover:bg-blue-700 px-2 py-1 rounded"
                >
                  Edit
                </button>
                <button
                  @click="handleDelete('galeri', item.id)"
                  class="text-xs text-white bg-red-600 hover:bg-red-700 px-2 py-1 rounded"
                >
                  Hapus
                </button>
              </div>
      ...
      <div v-if="activeTab === 'guru'">
        <form
          @submit.prevent="handleSubmit('guru')"
          class="mb-12 p-4 border rounded-lg bg-gray-50"
        >
          <h2 class="text-xl font-semibold text-gray-800 mb-4">
            {{ editMode ? "Edit Data Guru" : "Tambah Data Guru Baru" }}
          </h2>
          <div class="mb-4">
            <label for="guruNama" class="form-label"
          ...
          <div class="mb-4">
            <label :for="`guruFile`" class="form-label"
              >Pilih Foto (Kosongkan jika tidak ingin mengubah)</label
            >
            <input
              type="file"
              @change="handleFileChange($event, 'guru')"
              :id="`guruFile`"
              class="form-file-input"
            />
          </div>
        ...
        <div v-else class="space-y-4 max-h-96 overflow-y-auto pr-2">
          ...
          >
            ...
            <div class="flex space-x-3 flex-shrink-0 ml-4">
              <button
                @click="startEdit(guruData, 'guru')"
                class="text-sm text-blue-600 hover:underline"
              >
                Edit
              </button>
              <button
                @click="handleDelete('guru', guruData.id)"
                class="text-sm text-red-600 hover:underline"
              >
                Hapus
              </button>
      ...
      <div class="text-center mt-6">
        <router-link to="/" class="text-sm text-green-600 hover:underline"
          >← Kembali ke Website Utama</router-link
        >
      </div>
    </div>
  </div>
...
const loading = ref(false); // Untuk loading form submit
const loadingData = ref(false); // Untuk loading daftar data
...
const startEdit = (data, type) => {
  ...
  if (type === "guru")
    guru.value = { nama: data.nama, jabatan: data.jabatan, file: null };
  if (type === "galeri") galeri.value = { title: data.title, file: null };
  if (type === "berita")
    berita.value = { judul: data.judul, isi: data.isi, file: null };
  window.scrollTo(0, 0);
};
...
const cancelEdit = () => {
  ...
  ["sliderFile", "beritaFile", "galeriFile", "guruFile"].forEach((id) => {
...
const handleSubmit = async (type) => {
  ...
  try {
    ...
    switch (type) {
      case "slider":
        ...
        dataToSubmit = {
          ...
          order: Number(currentData.order),
        };
        break;
      case "berita":
    ...
    const docRef = isEditing
      ? doc(db, collectionName, editingId.value)
      : collection(db, collectionName);
...
.form-input {
  @apply w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500;
}
.form-file-input {
  @apply w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-green-100 file:text-green-700 hover:file:bg-green-200;
}
.form-button {
  @apply bg-green-600 hover:bg-green-700 text-white py-3 px-6 rounded-lg font-semibold transition-colors disabled:bg-gray-400;
}
.cancel-button {
  @apply bg-gray-500 hover:bg-gray-600 text-white py-3 px-6 rounded-lg font-semibold transition-colors;
}
.tab-button {
